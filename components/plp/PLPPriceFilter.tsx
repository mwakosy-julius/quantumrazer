...
export function PLPPriceFilter({ filters }: { filters: ProductFilters }) {
  ...
  const hi = filters.maxPrice ?? 4000;
  const [value, setValue] = useState([lo, hi]);

  useEffect(() => {
    setValue([filters.minPrice ?? 0, filters.maxPrice ?? 4000]);
  ...
  const hrefForRange = useMemo(
    () => (a: number, b: number) => {
      ...
      if (b < 4000) patch.maxPrice = b;
      if (a <= 0) patch.minPrice = undefined;
      if (b >= 4000) patch.maxPrice = undefined;
  ...
  return (
    <div className="pt-2">
      <Slider.Root
        ...
        onValueCommit={(v) => apply(v[0] ?? 0, v[1] ?? 4000)}
        max={4000}
        min={0}
        step={50}
